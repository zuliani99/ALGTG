...
def conv3x3Transposed(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1, output_padding: int = 0) -> nn.ConvTranspose2d:
...
def conv1x1Transposed(in_planes: int, out_planes: int, stride: int = 1, output_padding: int = 0) -> nn.ConvTranspose2d:
    """1x1 convolution
    """
...
class Resnet_Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.de_conv1 = nn.ConvTranspose2d(self.inplanes, 3, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = norm_layer(3)
        self.relu = nn.ReLU(inplace=True)
        self.unpool = nn.Upsample(scale_factor=4, mode='bilinear') # NOTE: invert max pooling
...
def ResNet18_Decoder() -> Resnet_Decoder:
    return Resnet_Decoder(BasicBlockDec, [2, 2, 2, 2]) 