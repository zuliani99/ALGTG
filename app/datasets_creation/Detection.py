...
def get_detection_dataset(ds_name: str, dataset_path: str):
    if ds_name == 'voc':
        pass
    return get_voc_data(dataset_path)
...
def get_voc_data(dataset_path:str) -> Tuple[VOCDetection, VOCDetection, VOCDetection]:
    ...
    train = VOCDetection(root=dataset_path,
                                    transform=SSDAugmentation(voc_config['min_dim'], voc_means))
    unlabeled = VOCDetection(root=dataset_path,
                                        transform=BaseTransform(300, voc_means),
                                        target_transform=VOCAnnotationTransform())
    test = VOCDetection(root=dataset_path,
...
class Det_Datasets():
    def __init__(self, dataset_name: str, init_lab_obs: int) -> None:
        ...
        self.transformed_trainset, self.non_transformed_trainset, self.test_ds = get_detection_dataset(dataset_name, '/content/VOCdevkit')
